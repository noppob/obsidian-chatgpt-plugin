...
var ChatGPTPlugin = class extends import_obsidian.Plugin {
  ...
  handleSelectionChange() {
    ...
    const editor = markdownView.editor;
    const selectedText = editor.getSelection();
    this.debugLogger.log("\u{1F4DD} handleSelectionChange", { length: (selectedText == null ? void 0 : selectedText.length) || 0 });
    if (selectedText && selectedText.trim()) {
      this.selectedTextCache = selectedText.trim();
      this.debugLogger.log("\u{1F4BE} Cached text", { length: this.selectedTextCache.length });
  ...
  showFloatingButton() {
    if (this.floatingButton)
      return;
    this.debugLogger.log("\u{1F4AC} showFloatingButton", { cacheLength: this.selectedTextCache.length });
    ...
    this.floatingButton.addEventListener("click", async () => {
      ...
      this.debugLogger.log("\u{1F5B1}\uFE0F Button clicked", { textLength: cachedText.length });
      this.hideFloatingButton();
      const markdownView = this.app.workspace.getActiveViewOfType(import_obsidian.MarkdownView);
      if (markdownView) {
        const editor = markdownView.editor;
...
`;
  ...
  async askChatGPTCustom(editor, cachedText) {
    this.debugLogger.log("\u{1F3AF} askChatGPTCustom called", { hasCached: !!cachedText, cachedLength: (cachedText == null ? void 0 : cachedText.length) || 0 });
    const markdownView = this.app.workspace.getActiveViewOfType(import_obsidian.MarkdownView);
    if (!markdownView) {
      new import_obsidian.Notice("Please open a note first");
      return;
    ...
    this.debugLogger.log("\u2705 Final selectedText", { length: (selectedText == null ? void 0 : selectedText.length) || 0, preview: selectedText == null ? void 0 : selectedText.substring(0, 50) });
    if (!selectedText || selectedText.trim() === "") {
      try {
        selectedText = await navigator.clipboard.readText();
        if (selectedText && selectedText.trim()) {
...
var InteractiveChatModal = class extends import_obsidian.Modal {
  constructor(app, plugin, selectedText, editor, readingModeInfo) {
    ...
    this.plugin.debugLogger.log("\u{1F3A8} Modal created", { selectedLength: selectedText.length, preview: selectedText.substring(0, 50) });
    this.messages.push({
      role: "system",
