...
export default class ChatGPTPlugin extends Plugin {
	...
	async callOpenAI(prompt: string): Promise<string> {
		...
		const response = await requestUrl({
			url: 'https://api.openai.com/v1/responses/create',
			method: 'POST',
			headers: {
		...
		// Responses APIではoutput_textまたはoutputフィールドを使用
		if (data.output_text) {
			return data.output_text.trim();
		}

		if (data.output && data.output.length > 0) {
			// outputが配列の場合、最初のアイテムからテキストを抽出
			const firstOutput = data.output[0];
			if (firstOutput.text) {
				return firstOutput.text.trim();
			}
			if (firstOutput.content) {
				return firstOutput.content.trim();
			}
		}
